...
def transferitm(request, item_id):
    ...
    transaction = Transaction(quantity=quantity, item=item, client=client, selling_price=selling_price, user=user,
                              discount=discount, type=type, profit=profit, TVA=TVA)
    transaction.save()
...
def returnitm(request, item_id):
    ...
    transaction = Transaction(quantity=quantity, item=item, client=client, selling_price=selling_price, user=user,
                              discount=discount, type=type, profit=profit)
    transaction.save()
    item.quantity = item.quantity + int(quantity)
    item.save()
